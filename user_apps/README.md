...
cmake -S user_apps -B build-user-apps-feature-only \
  ...
  -DAdaptiveAutosarAP_DIR=/opt/autosar_ap/lib/cmake/AdaptiveAutosarAP \
...
2. Register a new executable in `user_apps/CMakeLists.txt` and link imported
   AUTOSAR AP targets (`AdaptiveAutosarAP::ara_core`, `AdaptiveAutosarAP::ara_com`, etc).
