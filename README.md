...
| `ara::per` | ~75 % | Implemented (subset) | `KeyValueStorage`, `FileStorage`, `ReadAccessor` (with `Seek`/`GetCurrentPosition`), `ReadWriteAccessor` (with `Seek`/`GetCurrentPosition`), `SharedHandle`/`UniqueHandle` wrappers, `RecoverKeyValueStorage`/`ResetKeyValueStorage`, `RecoverFileStorage`/`ResetFileStorage`, `UpdatePersistency` (UCM schema-migration hook, SWS_PER_00456, schema metadata update + backup snapshot generation) | Full redundancy/recovery policy orchestration, schema versioning runtime |
