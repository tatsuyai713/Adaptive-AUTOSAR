...
### 100% にならない理由 (Why some modules cannot reach 100%)

The following table lists the reasons why each module cannot fully reach 100% of the AUTOSAR AP SWS specification. These gaps require hardware, OS-level daemons, or production ECU infrastructure that is outside the scope of this educational implementation.

| Module | Remaining gap | Reason |
| --- | --- | --- |
| `ara::core` | Deep OS-level runtime integration | Requires production POSIX signal handling, per-process memory isolation, and platform-specific abort/exception hooks |
| `ara::com` common API | Generated API stubs, full SWS corner cases | Full AUTOSAR SWS_CM code generation (compile-time proxy/skeleton stubs from ARXML) and all edge-case conformance testing require vendor toolchain integration |
| `ara::phm` | Platform IPC communicator, resident PHM daemon | Requires a resident process that monitors all supervised entities via IPC, and hardware watchdog (WDG) integration |
| `ara::per` | Redundant storage device/partition | SWS_PER requires redundant storage on separate physical devices; cannot be implemented without multi-device hardware |
| `ara::crypto` | Hardware Security Module (HSM) integration | SWS_CRYPT HSM-backed key storage, secure key generation, and tamper-resistant operations require dedicated HSM hardware |
| `ara::diag` | FlexRay/LIN transport, resident diagnostic daemon | Requires physical FlexRay/LIN bus hardware and a resident diagnostic manager daemon process |
| `ara::exec` | Cluster-level monitoring daemon | SWS_EM cluster-wide process monitoring requires a distributed supervisor across multiple ECUs |
| `ara::tsync` | GPS/GNSS hardware, cluster time calibration | SWS_TimeSync GPS/GNSS-based time source requires GPS receiver hardware; cluster-wide time calibration requires multi-ECU coordination |
| `ara::sm` | Per-interface OS network control, resident SM daemon | Full SM requires OS-level per-interface network bring-up/tear-down and a persistent state machine daemon |
| `ara::iam` | Biometric sensor hardware | SWS_IAM multi-factor authentication with biometric verification requires physical sensor hardware (fingerprint, face recognition) |
| `ara::nm` | CAN-TP/FlexRay transport adapter drivers | Real NM PDU transport requires CAN-TP (ISO 15765-2) or FlexRay hardware drivers and a resident NM coordinator daemon |
| `ara::com::secoc` | Cross-ECU freshness counter synchronization | SWS_SecOC cross-ECU monotonic counter sync requires a distributed protocol between multiple ECUs |
| `ara::ucm` | Secure boot/TPM, installer daemon | SWS_UCM secure boot verification via TPM and a resident installer daemon process require dedicated hardware and OS integration |
