...
```
/opt/autosar-ap/                     ← インストール先 (AUTOSAR_AP_PREFIX)
...
/opt/autosar-ap/user_apps_build/     ← ユーザーアプリビルド出力先
...
# ステップ 3: デプロイ資材をコピー
sudo cp -a deployment/rpi_ecu /opt/autosar-ap/deployment/
sudo cp -a configuration/. /opt/autosar-ap/configuration/
```
...
# ライブラリの確認
ls /opt/autosar-ap/lib/libara_*.so

# ARXML マニフェストの確認
ls /opt/autosar-ap/configuration/*.arxml
```
...
テンプレート変数:
- `__AUTOSAR_AP_PREFIX__` → `/opt/autosar-ap`
- `__USER_APPS_BUILD_DIR__` → `/opt/autosar-ap/user_apps_build`
...
raspi_ecu,12345,,0,AdaptiveAutosar/UserApps/raspi_ecu,5,60000,/opt/autosar-ap/user_apps_build/src/apps/basic/autosar_user_raspi_ecu_app --period-ms=100
```
...
```bash
# bringup.sh を直接実行
export AUTOSAR_USER_APPS_BUILD_DIR=/opt/autosar-ap/user_apps_build
...
`/opt/autosar-ap/user_apps/` にソーステンプレートがインストールされています:

```bash
ls /opt/autosar-ap/user_apps/src/apps/basic/
...
```cpp
// /opt/autosar-ap/user_apps/src/apps/basic/my_vehicle_app.cpp
...
```cmake
# /opt/autosar-ap/user_apps/src/apps/basic/CMakeLists.txt に追記
...
# バイナリが存在するか確認
ls -la /opt/autosar-ap/bin/<binary-name>

# ライブラリが見つかるか確認
ldd /opt/autosar-ap/bin/<binary-name>
```
